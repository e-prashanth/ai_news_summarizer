...
This repository generates one Markdown digest of AI news per day. It gathers only configured, reputable sources, filters for the previous calendar day in `NEWS_TIMEZONE`, summarizes with OpenAI, optionally emails the result, and stores the report in `Dailynew/`.
...
Run `python -m unittest discover -s tests -v` after behavior changes. For a manual smoke test without calling OpenAI, test collector/filter functions with fixture XML. A full run needs `OPENAI_API_KEY` and network access.
